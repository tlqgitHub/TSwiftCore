...
extension UIView {
    ...
    public func toastErrorCode(_ errorCode: Int){
        self.toast(TSwiftCore.alertmsg[errorCode]?.msgtitle ?? "")
    ...
    public func toastCenterErrorCode(_ errorCode: Int){
        self.toastCenter(TSwiftCore.alertmsg[errorCode]?.msgtitle ?? "")
    ...
    public func toastBottomErrorCode(_ errorCode: Int){
        self.toastBottom(TSwiftCore.alertmsg[errorCode]?.msgtitle ?? "")
    ...
    public func toastError(_ error:MikerError){
        if error.code == TSwiftCore.sharedInstance.toLoginErrorCode {
            self.toastCompletion(error.message){ _ in
                 ...
                 */
                if TSwiftCore.sharedInstance.isHasToLoginVc == false {
                    _ = "login".openURL()
                }
            }
        }else if error.code == TSwiftCore.sharedInstance.toForcedupdatingErrorCode{
            /*
            表示版本强制更新
             */
            if TSwiftCore.sharedInstance.isHasForcedupdating == false {
                TSwiftCore.sharedInstance.isHasForcedupdating = true
                _ = "forcedupdating".openURL(["message":error.message])
            }
            
        }else{
            if TSwiftCore.sharedInstance.isDebug {
                ...
                if code.hasPrefix("2") {
                    ...
                    self.toast(TSwiftCore.sharedInstance.errorMsg)
                }
            }
        }
    }
        ...
        if error.code == TSwiftCore.sharedInstance.toLoginErrorCode {
            ...
            if TSwiftCore.sharedInstance.isDebug {
                ...
                if code.hasPrefix("2") {
                    ...
                    self.toastCompletion(TSwiftCore.sharedInstance.errorMsg, completion: completion)
                }
            }
        }
    }
    ...
    public var rx_showerrorpage: AnyObserver<MikerError> {
        return Binder(self) { baseVc, error in
            ...
            if code.hasPrefix("2") {
                ...
                if let emptyView = baseVc.view.viewWithTag(TSwiftCore.ErrorPageViewTag) as? ErrorPageView {
